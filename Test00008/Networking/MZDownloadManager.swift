...
open class MZDownloadManager: NSObject {
    ...
    fileprivate func backgroundSession(identifier: String) -> URLSession {
        ...
        // --> therefore it is a good idea to creat this singleton object
        // and this way, keep the delegate alive as long as the App runs
        // (preventing nasty delegation-losses in VC-segue's etc...)
        ...
        for (index, downloadModel) in self.downloadingArray.enumerated() {
            if downloadTask.isEqual(downloadModel.task) {
                DispatchQueue.main.async(execute: { () -> Void in
                    ...
                    self.downloadDelegate?.downloadRequestDidUpdateProgress(downloadModel, index: index)
                })
                break
            }
        }
    ...
    func URLSession(_ session: Foundation.URLSession, task: URLSessionTask, didCompleteWithError error: NSError?) {
        ...
        DispatchQueue.main.async {
            if (error?.userInfo[NSURLErrorBackgroundTaskCancelledReasonKey] as? NSNumber)?.intValue == NSURLErrorCancelledReasonUserForceQuitApplication || (error?.userInfo[NSURLErrorBackgroundTaskCancelledReasonKey] as? NSNumber)?.intValue == NSURLErrorCancelledReasonBackgroundUpdatesDisabled {
                ...
                self.downloadDelegate?.downloadRequestDidPopulatedInterruptedTasks(self.downloadingArray)
                ...
                for(index, object) in self.downloadingArray.enumerated() {
                    ...
                    if task.isEqual(downloadModel.task) {
                        if error?.code == NSURLErrorCancelled || error == nil {
                            ...
                            if error == nil {
                                self.downloadDelegate?.downloadRequestFinished?(downloadModel, index: index)
                            } else {
                                self.downloadDelegate?.downloadRequestCanceled?(downloadModel, index: index)
                            }
...
extension MZDownloadManager {
    ...
    public func addDownloadTask(_ fileName: String, fileURL: String) {
        self.addDownloadTask(fileName, fileURL: fileURL, destinationPath: "")
    }
    ...
    public func pauseDownloadTaskAtIndex(_ index: Int) {
        ...
        self.downloadDelegate?.downloadRequestDidPaused?(downloadModel, index: index)
    }
    ...
    public func resumeDownloadTaskAtIndex(_ index: Int) {
        ...
        self.downloadDelegate?.downloadRequestDidResumed?(downloadModel, index: index)
    }
    ...
    public func cancelTaskAtIndex(_ index: Int) {
        ...
        let downloadInfo = self.downloadingArray[index]
